---
title: Изменить статус оплаты
order: 19.1
---

В системе доступно массовое действие «Изменить статус оплаты обучения»: после успешного проведения массового действия у выбранных заявок измениться статус оплаты ровно так же, как если бы его меняли по одной заявке из карточки, с теми же ограничениями и с теми же последствиями для этапа заявки.

На странице со списком заявок, в выпадающем списке «Выберите действие» есть пункт «Изменить статус оплаты».

![](./izmenit-status-oplaty.webp){width=1045px height=665px}

Далее в новой вкладке открывается страница выбранного массового действия с информацией, активной кнопкой «Выполнить» и счётчиком.

![](./izmenit-status-oplaty-2.webp){width=827px height=479px}

Когда действие выполнится, то кнопка «Выполнить» изменится на «Завершено». Обратите внимание на результат обработанных заявок, исправьте ошибки при необходимости и повторите массовое действие для этих заявок.