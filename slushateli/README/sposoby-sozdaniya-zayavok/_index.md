...
**\-** либо же [**дополнительные поля **](./../../../Organization/dopolnitelnye-dokumenty-i-polya-vvoda-dannykh/_index)(если вам нужно, чтобы обучающиеся сами заполняли какую-либо информацию в своих ЛК и она в дальнейшем показывалась в карточках заявок).
...
По каждому источнику заполнить:

1) само название источника;

2) короткое название на английском языке (не использовать ничего, кроме английских букв,

его мы далее будем вставлять в код формы)

3) выбрать значение «*Заявка гражданина*» в поле «*В каком разделе Flow будет использоваться?*»

4) проставить при необходимости галочку «*Должно быть уникальным значением*» -

если нужно, чтобы каждый раз, когда кто-то заполняет форму, в это поле можно было ввести

только уникальную информацию, например, какой-нибудь промокод

5) не проставляем галочку «Скрыто в интерфейсе», если нужно, чтобы дополнительная

информация выводилась в заявке.
...
После добавления внешних источников и/или дополнительных полей, следует внести некоторые изменения в код формы из Шага 3 основной инструкции. Рассмотрим на примере только что созданного источника «Промокод»:
...
Далее работу с формой можно продолжать с Шага 4 основной инструкции.
