...
Приказы фиксируют факт зачисления слушателя на обучение и его отчисления по итогам. Выпускаются тремя способами -- автоматически или вручную -- в зависимости от настроек группы шаблонов.
...
**Удалить приказы из Flow нельзя.**

**Для ручного выпуска можно  заменить приказ  в заявке слушателя (предварительно создав новый приказ)/заменить скан приказа в списке приказов.**
...
<div class="pw">
  <div class="pw-controls">
    ...
    <div id="pw-launch-col">
      ...
      <div class="pw-group">
        <button class="pw-btn active" onclick="pwSet('launch','night',this)">Запускается сам (ночью перед днем старта потока)</button>
...
var sc = {
  'auto-night-enroll': {label:'Автоматический — ночной запуск — зачисление',lc:'#0C447C',title:'Приказ генерируется автоматически в день старта потока',steps:[
    ...
    {n:2,bg:'#E6F1FB',c:'#0C447C',t:'Убедитесь что слушатели одобрили ДЗС до старта потока',s:'Заявление, согласие на обработку ПД, договор (если требуется)'},
  ...
  ],note:'Для добегающих слушателей система ежедневно создаёт отдельные приказы с новыми номерами (ПЗ-2, ПЗ-3...).'},
  ...
  'auto-night-dismiss-success': {label:'Автоматический — ночной запуск — успешное завершение',lc:'#085041',title:'Приказ генерируется ночью после дня завершения обучения',steps:[
    {n:1,bg:'#E6F1FB',c:'#0C447C',t:'В группе шаблонов установите галочку «Автоматически» в разделе приказов'},
    {n:2,bg:'#E6F1FB',c:'#0C447C',t:'Дождитесь окончания потока и выставьте статус «Завершил успешно»',s:'На странице заявки в блоке "Обучение"/«Завершите обучение» через Odin'},
  ...
  'auto-night-dismiss-fail': {label:'Автоматический — ночной запуск — неуспеваемость',lc:'#633806',title:'Приказ генерируется ночью после дня завершения обучения',steps:[
    {n:1,bg:'#E6F1FB',c:'#0C447C',t:'В группе шаблонов установите галочку «Автоматически» в разделе приказов'},
    {n:2,bg:'#E6F1FB',c:'#0C447C',t:'Дождитесь окончания потока и выставьте статус «Не завершил обучение»',s:'На странице заявки в блоке "Обучение"/ «Завершите обучение» через Odin'},
  ...
  'auto-potok-enroll': {label:'Автоматический — со страницы потока — зачисление',lc:'#0C447C',title:'Запускаете генерацию досрочно из карточки потока',steps:[
    ...
    {n:2,bg:'#E6F1FB',c:'#0C447C',t:'Убедитесь что слушатели одобрили ДЗС',s:'Заявление, согласие на обработку ПД, договор (если требуется)'},
  ...
  ],note:'Для добегающих запустите генерацию повторно — они попадут в отдельный приказ с новым номером.'},
  ...
  'auto-potok-dismiss-success': {label:'Автоматический — со страницы потока — успешное завершение',lc:'#085041',title:'Запускаете генерацию досрочно из карточки потока',steps:[
    {n:1,bg:'#E6F1FB',c:'#0C447C',t:'Выставьте статус «Завершил успешно» по слушателям',s:'На странице заявки в блоке "Обучение"/«Завершите обучение» через Odin'},
  ...
  'auto-potok-dismiss-fail': {label:'Автоматический — со страницы потока — неуспеваемость',lc:'#633806',title:'Запускаете генерацию досрочно из карточки потока',steps:[
    {n:1,bg:'#E6F1FB',c:'#0C447C',t:'Выставьте статус «Не завершил обучение» по слушателям',s:'На странице заявки в блоке "Обучение"/«Завершите обучение» через Odin'},
    {n:2,bg:'#E6F1FB',c:'#0C447C',t:'Откройте карточку потока — «...» — «Сгенерировать приказ на отчисление»'},
  ...
  'manual-enroll': {label:'Ручной — зачисление',lc:'#0C447C',title:'Создаёте приказ и назначаете слушателям вручную',steps:[
    ...
    {n:3,bg:'#E6F1FB',c:'#0C447C',t:'Заполните: номер, программа, поток, тип «На зачисление», основание - заявление слушателя, дата'},
  ...
  'manual-dismiss-success': {label:'Ручной — успешное завершение',lc:'#085041',title:'Создаёте приказ после завершения обучения',steps:[
    ...
    {n:2,bg:'#E6F1FB',c:'#0C447C',t:'Тип «Приказ об отчисление», основание «Выдача документа о квалификации»'},
  ...
  'manual-dismiss-fail': {label:'Ручной — неуспеваемость',lc:'#633806',title:'Создаёте приказ после завершения обучения',steps:[
    {n:1,bg:'#E6F1FB',c:'#0C447C',t:'Перейдите в «Обучение» — «Приказы» — «Создать»'},
    {n:2,bg:'#E6F1FB',c:'#0C447C',t:'Тип «Приказ об отчисление», основание «Неуспеваемость»'},
    {n:3,bg:'#E6F1FB',c:'#0C447C',t:'Прикрепите скан приказа'},
  ...
  'manual-dismiss-own': {label:'Ручной — по заявлению слушателя',lc:'#4A1B0C',title:'Создаёте приказ после одобрения заявления',steps:[
    ...
    {n:2,bg:'#E6F1FB',c:'#0C447C',t:'Перейдите в «Обучение» — «Приказы» — «Создать»'},
    {n:3,bg:'#E6F1FB',c:'#0C447C',t:'Тип «Приказ об отчисление», с основанием «Зявление слушателя», прикрепите скан'},
...
**Приказ на отчисление** может быть выпущен во время обучения, если слушатель написал заявление на отчисление, либо после фактической даты завершения обучения, когда в системе Odin будет выставлено значение успешно/неуспешно пройдено обучение.
