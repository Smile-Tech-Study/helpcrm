...
**Генерируется автоматически в день старта потока. На каждую группу шаблонов создаётся отдельный приказ.**

:::

| **Сценарий**                                          | **Когда генерируется**                                                    | **Дата в приказе** | **Кто попадает**                                                                                                 |
|-------------------------------------------------------|---------------------------------------------------------------------------|--------------------|------------------------------------------------------------------------------------------------------------------|
| Стандартное зачисление                                | В день старта потока в 00:30 мск                                          | Дата старта потока | Все слушатели потока с одобренными ДЗС на момент генерации. На каждую группу шаблонов создаётся отдельный приказ |
| Ранний доступ через Odin (до старта потока)           | В день старта потока в 00:30 мск                                          | Дата старта потока | Слушатели, которым дан ранний доступ до старта потока                                                            |
| Слушатели с поздним зачислением (после старта потока) | Ежедневно после старта - отдельный приказ с новым номером (ПЗ-2, ПЗ-3...) | Дата старта потока | Слушатели, получившие одобренные ДЗС после выпуска основного приказа                                             |



:::info 

ДЗС -- документы на зачисление слушателя: заявление, согласие на обработку персональных данных и договор (если договор требуется для данной заявки). В шаблоне приказа перечислены все включённые слушатели
...
|--------------------------------------------------|------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------|
| По заявлению слушателя                           | Раз в сутки: в день одобрения заявления на отчисление (или на следующий, если задача уже отработала в этот день) | Слушатели, у кого есть одобренное и подписанное заявление на отчисление и ещё нет приказа на отчисление. На каждую группу шаблонов создаётся отдельный приказ |
| За неуспеваемость                                | Ночью после дня завершения обучения в потоке                                                                     | Слушатели, со статусом «Не завершил обучение», и  у кого ещё нет приказа на отчисление. На каждую группу шаблонов создаётся отдельный приказ                  |
| С выдачей документа о квалификации (сертификата) | Ночью после дня завершения обучения в потоке                                                                     | Слушатели со статусом «Завершил успешно» у кого ещё нет приказа на отчисление. На каждую группу шаблонов создаётся отдельный приказ                           |

Если в группах шаблонов **используется единый текст** для всех типов приказов на отчисление, дополнительных **действий не требуется** -  система продолжит работать в обычном режиме.  Если **текст приказа отличается**, необходимо зайти в [группу шаблонов](./../../Organization/Shablony/_index) и **отредактировать нужный шаблон**.

![](./avtomaticheskii-vypusk.webp){width=1920px height=856px}
  ...
  <table class="pa-table">
    ...
    <tbody>
      <tr>
        ...
        <td>
          <div class="pa-desc">Открывает сгенерированный документ приказа.</div>
        </td>
      </tr>
      <tr>
        <td>
      ...
      <tr>
        <td>
          <div class="pa-icon" style="background:#FAEEDA">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#854F0B" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>
          </div>
        </td>
